...
mamy blad z przenoszeniem wyrazenia, na ktorym jest kursor
- ale chyba mamy tez jakies wyobrazenie skad sie on bierze

W kazdym razie tym sie dzisiaj jeszcze nie przejmujemy
(choc docelowo bedzie trzeba: przy wyodrebnianiu wyrazenia
bedziemy musieli sprawdzic, czy kursor jest jego czescia,
i jezeli tak, to go przeniesc do overlaya)

zamiast tego chcielibysmy w ogole usunac wlasciwosci
null-head-space i null-tail-space, a zamiast nich
uzyc EmptyListProxy

Zmiany przebiegalyby w nastepujacy sposob:
1. dodajemy do parsera konstrukcje EmptyListProxy
2. usuwamy odniesienie do null-head-space i null-tail-space
z traverse

Wydaje sie, ze podmianka na EmptyListProxy poszla
gladko. Mamy jeszcze 3 dni commitowania:

1. czwartek - dodajemy dzisiejsze zmiany (i to koniec)
2. piatek - tworzymy repozytorium github.com/panicz/grasp
i dodajemy do niego zrodla (+ plik README.org)
3. sobota - aktualizujemy plik README.org w grasp-android,
archiwizujemy repozytorium, piszemy notatki w mediach
spolecznosciowych

* 27.12.2022

Udalo sie dodac edycje ResizeBox.

Mamy tez dosc brzydkawego buga przy przenoszeniu
elementow (jezeli dany element to pierwszy element
listy, sam bedacy lista).

Tym bysmy sie jeszcze dzis zajeli i jutro walneli
commita.

Ponadto mamy taki problem, ze na pustych nawiasach
nie dziala ani Remove, ani ResizeBox, i to takze
dobrze by bylo naprawic.

Natomiast kolejne rzeczy, ktore chcemy zrobic, to:
- edycja dokumentu z klawiatury (wraz z historia edycji)
- obsluga komentarzy
- dzielenie ekranu
- scrollowanie widoku
- otwieranie i zapisywanie dokumentu

WALKA Z DZIWNYM BLEDEM:

przede wszystkim musimy zrozumiec, skad sie ow blad bierze.

(prawdopodobnie stad, ze renderowanie ustawia screen-position)

OK,

ale teraz (przy okazji proby zmiany rozmiaru)
pojawila sie jeszcze jedna koncepcja, mianowicie
- zeby usunac "null-head-space" oraz "null-tail-space",
i tak jak mamy "dublerowanie" metod getCar i getCdr
w cons dla typu Atom, tak chcielibysmy zrobic to samo
dla typu EmptyListProxy

* 22.12.2022

dzis (i ew. jutro) sprobowalibysmy dodac
opetacje Resize do historii

Operacje nazwiemy ResizeBox - po pierwsze
zeby lepiej brzmialo, ale po drugie tez zeby
odroznic od Resize
...
(ResizeBox at: Cursor
	   ...
	   from-height: real
	   to-height: real
	   anchored-at: LineEnding)

ok, fajnie i co dalej?

powinno to dzialac tak, ze:
- mamy funkcje zmieniajace rozmiar pudelka
  poza obiektem Resize
- metoda move! w Resize wywoluje te funkcje
- metoda drop! w Resize dodaje do historii
  obiekt ResizeBox
- metoda apply! w ResizeBox rowniez robi
  to samo

	   
* 21.12.2022

teraz bysmy chcieli te operacje zintegrowac
- najpierw z mechanizmem drag&drop, a nastepnie
z operacjami edycji z klawiatury

jednak wydaje sie, ze w obu przypadkach najlepiej
byloby dokonywac edycji na komputerze

OK, zatem komputer it is.

I co daley?

Po pierwsze, wyglada na to, ze te nasze ostatnie
grzebaniny troche zepsuly, bo nowa linia w spacji
za dodanym elementem jest usuwana. Trzeba to bedzie
naprawic.

Po drugie, chcielibysmy rejestrowac (i moc cofac)
rowniez zdarzenia zmiany rozmiaru pudelka

(ResizeBox from: Extent to: Extent)

Po trzecie, chcielibysmy dodac operacje pisania
i dodawania nowych symboli.

* 20.12.2022

Mamy juz cos co wyglada jak jakies operacje edycji
